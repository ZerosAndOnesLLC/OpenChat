...
function ChannelItem({
  ...
  const encryptionEnabled = useWebSocketStore((state) =>
    state.channels.find(ch => ch.id === channel.id)?.encryption_enabled
  );
          ...
          <div className="flex items-center min-w-0 flex-1">
            ...
            {encryptionEnabled && (
              <span title="Encrypted">
                <Shield className="ml-1 h-3 w-3 flex-shrink-0 text-green-400" />
              </span>
            )}
            {channelCall && (
